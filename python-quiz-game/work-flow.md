...
- [x] 진행한 보너스 기능 전체 시나리오 재테스트: 랜덤 순서 확인(셔플 전/후 출제 순서가 목록 순서와 다름을 확인, 원본 `self.quizzes` 순서는 유지), 문제 수 제한 동작(`_read_choice`로 1~전체 문제 수 검증), 힌트 사용 시 감점 확인(힌트 사용 후 정답 시 0.5점 처리 — 7문제 중 6문제 정답(그 중 1문제는 힌트 사용)으로 5.5/7×100=78.57→79점이 정확히 계산됨을 확인), 퀴즈 삭제 후 목록/저장 확인(삭제 후 목록 개수 감소 및 재조회 시 반영 확인), 히스토리 누적·재실행 후 유지 확인(플레이할 때마다 `history`에 기록 추가, 점수 확인 메뉴에 최근 기록 노출, 프로세스 재시작 후에도 `state.json`에서 그대로 복원)
  - 추가로 손상된 `state.json`(깨진 JSON) 로드 시 안내 메시지 출력 후 기본 데이터로 정상 폴백하는지 확인
- [x] `python -m py_compile`로 전체 파일 컴파일 확인 (`main.py`, `quiz.py`, `quiz_game.py` 모두 통과)
- [x] README의 기능 목록/메뉴 구성/코드 구조/`state.json` 스키마 설명에 진행한 보너스 기능 반영 (메뉴에 "퀴즈 삭제" 추가, `Quiz`/`QuizGame` 코드 구조 설명에 `hint`/`history`/`delete_quiz`/`_read_answer_choice` 반영, `state.json` 예시 JSON에 `hint`/`history` 필드 추가)
