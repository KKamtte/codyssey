...
### 14단계. 랜덤 출제
- [ ] `quiz_game.py` 상단에 `import random` 추가
- [ ] `play_quiz`에서 `self.quizzes`를 직접 섞지 말고 복사본(`list(self.quizzes)`)을 만들어 `random.shuffle()`로 섞은 뒤 그 복사본을 출제 순서로 사용 (원본 리스트 순서·번호는 그대로 유지되어야 목록 보기(3번 메뉴)가 영향받지 않음)
- [ ] 커밋: `feat: 퀴즈 풀기 랜덤 출제 기능 추가`
