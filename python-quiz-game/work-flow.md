...
### 15단계. 문제 수 선택
- [ ] `play_quiz` 시작 시 "몇 문제를 풀지" 입력받기 (`_read_choice`를 재사용해 `1 ~ len(self.quizzes)` 범위로 검증)
- [ ] 14단계에서 섞은 리스트의 앞에서부터 선택한 문제 수만큼 슬라이싱해 출제 (`total`도 전체 문제 수가 아니라 이번에 실제로 푼 문제 수로 계산되도록 수정)
- [ ] 퀴즈가 1개뿐인 경우 등 입력 자체를 건너뛰어도 되는 예외 상황 처리
- [ ] 커밋: `feat: 퀴즈 풀기 문제 수 선택 기능 추가`
