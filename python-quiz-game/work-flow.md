...
### 17단계. 퀴즈 삭제 기능
- [ ] 메뉴에 "퀴즈 삭제" 항목 추가, `MENU_*` 상수와 종료 번호를 한 칸씩 밀어 재정렬 (예: 6.퀴즈 삭제, 7.종료)
- [ ] `list_quizzes`와 동일하게 목록을 보여준 뒤 삭제할 번호를 `_read_choice`로 입력받아 `self.quizzes`에서 제거
- [ ] 퀴즈가 없는 경우 "삭제할 퀴즈가 없습니다" 안내 후 메뉴로 복귀
- [ ] 삭제 후 `self.save()` 호출로 `state.json`에 반영
- [ ] 커밋: `feat: 퀴즈 삭제 기능 추가`
