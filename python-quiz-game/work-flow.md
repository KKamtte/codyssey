...
### 16단계. 힌트 기능
- [ ] `quiz.py`의 `Quiz` 클래스에 `hint` 속성 추가 (`__init__`, `to_dict`, `from_dict` 모두 반영)
- [ ] 기존 `state.json`(힌트 필드 없음)을 불러올 때 깨지지 않도록 `from_dict`에서 `data.get("hint", "")`처럼 기본값 처리
- [ ] `DEFAULT_QUIZZES` 각 문제에 힌트 문구 추가
- [ ] `play_quiz`의 정답 입력 프롬프트에서 `h`(또는 `힌트`) 입력 시 힌트를 출력하고 사용 여부를 문제별로 기록한 뒤 다시 정답 입력을 받도록 처리 (`_read_choice`와 별도로 힌트 분기를 먼저 확인하는 입력 루프 필요)
- [ ] 힌트를 사용하고 맞힌 문제는 감점된 배점(예: 정상 1점 대비 0.5점)만 인정하도록 점수 계산 로직 변경 — 문제별 배점을 합산한 뒤 `total`로 나눠 최종 점수(%)를 산출하는 방식으로 `score` 계산을 리팩터링
- [ ] `show_score`/`state.json` 표기에 "힌트 사용 문제 수" 등 필요한 정보가 있으면 함께 노출할지 결정
- [ ] 커밋: `feat: 힌트 기능 및 힌트 사용 시 점수 차감 로직 추가`
